...
const BookDetails = () => {
  ...
  if (!book) {
    return (
      <div className="text-center mt-10 text-red-500 dark:text-red-400 text-xl">
        Book not found.
      </div>
    );
  }
  ...
  return (
    <div className="pt-15">
      <div className="w-11/12 sm:w-10/12 md:w-9/12 lg:w-8/12 mx-auto mt-10 p-6 rounded-xl shadow-2xl bg-[#f3fce7] dark:bg-gray-900">
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-1 space-y-2 text-[#2e3810] dark:text-gray-100">
            <h2 className="text-3xl font-bold">{book.title}</h2>
            <p><span className="font-semibold">Author:</span> {book.author}</p>
            <p><span className="font-semibold">Total Pages:</span> {book.pages}</p>
            ...
            <p><span className="font-semibold">Overview:</span> {book.overview}</p>

            <div className="mt-4 bg-[#e0e8c9] dark:bg-gray-800 p-4 rounded-lg">
              <h3 className="text-lg font-bold mb-2">Uploaded By</h3>
              <p><span className="font-semibold">Name:</span> {book.name}</p>
              <p><span className="font-semibold">Email:</span> {book.email}</p>
            </div>
            ...
            <div className="mt-6 flex items-center justify-between">
              ...
              {!isOwner && user && (
                <button
                  onClick={upvoteBook}
                  disabled={upvoting}
                  className="px-4 py-2 bg-[#4F7942] text-white rounded-full hover:bg-[#808000] dark:bg-gray-800 dark:hover:bg-gray-700 disabled:opacity-50"
                >
        ...
        <div className="mt-8">
          ...
          {book.reviews && book.reviews.length > 0 ? (
            ...
            <p className="dark:text-gray-300">No reviews yet. Be the first to review!</p>
          )}
        </div>
        ...
        {user && (
          <div className="mt-4">
            ...
            {editingReview ? (
              <button
                onClick={editReview}
                className="mt-2 px-6 py-2 bg-gradient-to-r from-[#4F7942] to-[#808000] dark:from-gray-800 dark:to-gray-700 dark:hover:from-gray-800 dark:hover:to-gray-600 text-white"
              >
                Edit Review
              </button>
            ) : (
              <button
                onClick={postReview}
                className="mt-2 px-6 py-2 bg-gradient-to-r from-[#4F7942] to-[#808000] dark:from-gray-800 dark:to-gray-700 dark:hover:from-gray-800 dark:hover:to-gray-600 text-white rounded-full"
              >
                Submit Review
              </button>
            )}
        ...
        <div className="mt-8 text-center">
          <button
            onClick={() => navigate("/")}
            className="px-6 py-2 bg-[#808000] hover:bg-[#6b6b00] dark:bg-gray-800 dark:hover:bg-gray-700 text-white rounded-full transition duration-300"
          >
            Back to Home
          </button>
        </div>
